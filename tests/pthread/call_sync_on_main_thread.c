/*
 * Copyright 2017 The Emscripten Authors.  All rights reserved.
 * Emscripten is available under two separate licenses, the MIT license and the
 * University of Illinois/NCSA Open Source License.  Both these licenses can be
 * found in the LICENSE file.
 */

#include <stdio.h>
#include <assert.h>
#include <string.h>
#include <stdlib.h>

extern const char *getDomElementContents(const char *domElementSelector);
extern int isThisInWorker(void);
extern int isThisOnMainRuntimeThread(void);
extern int receivesAndReturnsAnInteger(int i);

// Define this if compiling via -s PROXY_TO_PTHREAD=1
// #define PROXY_TO_PTHREAD 1

int main()
{
	char name[7] = "body";
	const char *dst = getDomElementContents(name);
	memset(name, 0, sizeof(name)); // Try to uncover if there might be a race condition and above line was not synchronously processed, and we could take name string away.
	int inWorker = isThisInWorker(); // Build this application with -s USE_PTHREADS=1 -s PROXY_TO_PTHREAD=1 for this to return 1, otherwise returns 0.
	int onMainRuntimeThread = isThisOnMainRuntimeThread(); // This should always return 1 due to the proxied call.
	int returnedInt = receivesAndReturnsAnInteger(4);
	printf("text: \"%s\". inWorker: %d, onMainRuntimeThread: %d, returnedInt: %d\n", dst, inWorker, onMainRuntimeThread, returnedInt);
	assert(strstr(dst, "<canvas"));
	free((void*)dst);
	assert(inWorker == PROXY_TO_PTHREAD);
	assert(onMainRuntimeThread == 1);
	assert(returnedInt == 42 + 4);
#ifdef REPORT_RESULT
	REPORT_RESULT(1);
#endif
}
