...
static int __pthread_timedjoin_np(pthread_t t, void **res, const struct timespec *at)
{
	...
	// TODO(kleisauke): Return EDEADLK when attempting to join itself? This is not tested in posixtestsuite, but was checked previously in JS.
#endif
...
#ifdef __EMSCRIPTEN__ // XXX Emscripten return EINVAL when attempting to join an already detached thread
	...
	if (state == DT_EXITING) emscripten_cleanup_thread(t);
